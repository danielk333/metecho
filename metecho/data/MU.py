...
def _get_header_data(fid):
    ...
    """
    header_data["record_start_time"] = np.datetime64(convert_date(
    ...
    end_time = decode_utf(np.fromfile(fid, dtype='S12', count=1, offset=84))
    temp_end_time = np.datetime64(start_date + " " + end_time, 'ns')
    header_data["record_end_time"] = fix_date_edge_case(header_data["record_start_time"], temp_end_time)
    """
    ...
    header_data["lag_number_per_ACF"] = np.fromfile(fid, dtype=np.int32, count=21, offset=172)
    """
    ...
    header_data["beam_direction_number"] = np.fromfile(fid, dtype=np.int32, count=16, offset=256)
    """
    ...
    header_data["hp_file_name"] = decode_utf(np.fromfile(fid, dtype='S8', count=1, offset=336))
    """
    ...
    header_data["number_of_sum_ACF"] = np.fromfile(fid, dtype=np.int32, count=16, offset=352)
    """
    ...
    header_data["observation_param_name"] = decode_utf(np.fromfile(fid, dtype='S16', count=1, offset=600))
    """
    ...
    header_data["tx_frequency"] = np.fromfile(fid, dtype=np.int32, count=5, offset=1696)
    """
    ...
    header_data["rx_attentuator"] = np.fromfile(fid, dtype=np.int32, count=4, offset=1724)
    """
    ...
    header_data["rx_state"] = np.fromfile(fid, dtype=np.int32, count=4, offset=1744)
    """
    ...
    header_data["sample_start_time"] = np.fromfile(fid, dtype=np.int32, count=256, offset=1820)
    """
    ...
    header_data["coherent_integrations"] = np.fromfile(fid, dtype=np.int32, count=29, offset=2964)
    """
    %-- (3081-3196) Number of FFT points for each combined channel:
    %-- 29 unsigned longwords -> 29 * 4 byte (4*8=32 bit) integers
    """

    header_data["fft_points"] = np.fromfile(fid, dtype=np.int32, count=29, offset=3080)
    """
    %-- (3197-3312) Number of data points for each combined channel:
    %-- 29 unsigned longwords -> 29 * 4 byte (4*8=32 bit) integers
    """

    header_data["data_points"] = np.fromfile(fid, dtype=np.int32, count=29, offset=3196)
    """
...
# month dictionary and the decode_utf function.


def convert_date(date_str):
    if type(date_str) == np.ndarray:
        date_str = decode_utf(date_str)
...
def fix_date_edge_case(start_time, end_time):
...
def convert_MUI_to_h5(in_path, out_path):
    ...
    #header_data = _get_header_data(stuff)
    #
